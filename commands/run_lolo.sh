        ...
        for lang in abs btk bew bhp jav mad mak min mui rej sun; do
            for task in sent; do
                for ((i = 0; i < ${#large_models[@]}; ++i)); do
                    python main_lolo.py --dataset_name $dset_name --task $task --lang $lang --model_checkpoint ${large_models[$i]} --n_epochs 100 --lr 1e-5 --train_batch_size 4 --eval_batch_size 4 --grad_accum 8 --seed 0 --num_sample $num_sample --device cuda &
