    ...
    if model_type == 'indo-gpt2':
        ...
        if not is_inference:
            ...
            hyps = model.generate(input_ids=enc_batch, attention_mask=enc_mask_batch, num_beams=beam_size if is_test else 1, 
                                    ...
                                    pad_token_id=tokenizer.pad_token_id, eos_token_id=tokenizer.eos_token_id, decoder_start_token_id=label_batch[:,0])
    # generate prediction & label list
    list_hyp = []
    list_label = []
...
def forward_generation_multilingual(model, batch_data, tokenizer, model_type, is_inference=False, is_test=False, 
                           skip_special_tokens=True, beam_size=1, max_seq_len=512, device='cpu', length_penalty=1.0, 
                           top_p=1.0, top_k=50, repetition_penalty=1.0, do_sample=False,  **kwargs):
    # Unpack batch data
    enc_batch = batch_data['input_ids']
    dec_batch = batch_data['decoder_input_ids']
    enc_mask_batch = batch_data['attention_mask']
    dec_mask_batch = batch_data['decoder_attention_mask']
    label_batch = batch_data['labels']
    token_type_batch = None
            
    if device == "cuda":
        enc_batch = enc_batch.cuda() if enc_batch is not None else None
        dec_batch = dec_batch.cuda() if dec_batch is not None else None
        enc_mask_batch = enc_mask_batch.cuda() if enc_mask_batch is not None else None
        dec_mask_batch = dec_mask_batch.cuda() if dec_mask_batch is not None else None
        label_batch = label_batch.cuda() if label_batch is not None else None
        token_type_batch = token_type_batch.cuda()  if token_type_batch is not None else None
    ...
    if model_type == 'indo-gpt2':
        ...
        # BART and T5 should go here!!
        if not is_inference:
            outputs = model(input_ids=enc_batch, attention_mask=enc_mask_batch, decoder_input_ids=dec_batch, labels=label_batch)
            loss, logits = outputs[:2]
            hyps = logits.topk(1, dim=-1)[1]
        else:
            loss = 0
            hyps = model.generate(input_ids=enc_batch, attention_mask=enc_mask_batch, num_beams=beam_size if is_test else 1, 
                        max_length=max_seq_len, early_stopping=True, length_penalty=length_penalty, use_cache=True,
                        pad_token_id=tokenizer.pad_token_id, eos_token_id=tokenizer.eos_token_id, decoder_start_token_id=int(dec_batch[0,0]))
    # generate prediction & label list
    list_hyp = []
    list_label = []
    for j in range(len(hyps)):
